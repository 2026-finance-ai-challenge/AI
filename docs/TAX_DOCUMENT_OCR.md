...
Tesseract TSV의 단어를 페이지·블록·문단·줄 번호로 묶어 원래 파서가 기대하는 줄 단위 입력을 보존한다. 각 OCR 자식 프로세스에만 OpenMP 스레드 1개와 20초 제한을 적용한다. RAG 등 다른 프로세스의 스레드는 변경하지 않는다. 영역 경계에서 잘린 영문 이름 라벨도 제거하여 실제 이름 값을 버리지 않는다. 처리 시간 로그에는 문서 종류와 소요 시간만 기록한다.
